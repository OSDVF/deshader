...
instrument_clients: std.ArrayListUnmanaged(declarations.instruments.InstrumentClient) = .empty,
...
pub fn addInstrumentClient(self: *Service, i: declarations.instruments.InstrumentClient) !void {
...
pub fn toOpaque(self: *@This()) *declarations.instruments.Service {
    return @ptrCast(self);
}
...
pub const State = struct {
    ...
    channels: Processor.Result.StageChannels,
    instruments: std.AutoArrayHashMapUnmanaged(u64, declarations.instruments.InstrumentClient) = .empty,
    ...
    pub fn asOpaque(self: *@This()) *declarations.instruments.State {
        return @ptrCast(self);
    }
};
...
pub fn addBreakpoint(
    self: *@This(),
    locator: storage.Locator(Shader.SourcePart),
        ...
        return std.fmt.allocPrint(
            ...
            .{
                ...
                storage.Locator(Shader.SourcePart).PartRef{
                    .ref = shader.ref,
                    .part = part_index orelse shader.getPartIndex() orelse unreachable,
                },
                shader.toExtension(),
            },
        );
        ...
        if (self.Shaders.all.get(state_entry.key_ptr.*)) |shader_parts| {
            ...
            try result.append(allocator, Running{
                ...
                .name = try std.fmt.allocPrint(
                    allocator,
                    "{name}",
                    .{Shader.SourcePart.WithIndex{ .source = shader, .part_index = 0 }},
                ),
...
fn getStateCache(self: *@This(), params: State.Params, ref: Shader.StageRef) !union(enum) {
    ...
    if (self.state.getPtr(ref)) |cached| {
        ...
        if (params.context.screen[0] != 0) {
            if (params.context.screen[0] != cached.params.context.screen[0] or
                params.context.screen[1] != cached.params.context.screen[1])
            {
        ...
        if (params.compute[0] != 0) {
            if (params.compute[0] != cached.params.compute[0] or params.compute[1] != cached.params.compute[1] or
                params.compute[2] != cached.params.compute[2])
            {
...
const stages_order = std.EnumMap(declarations.shaders.Stage, usize).init(.{
    .gl_vertex = 0,
    .vk_vertex = 0,
    .gl_tess_control = 1,
    .vk_tess_control = 1,
...
fn sortStages(_: void, a: ShadersRefMap.Entry, b: ShadersRefMap.Entry) bool {
    return (stages_order.get(a.value_ptr.*.items[0].stage) orelse 0) <
        (stages_order.get(b.value_ptr.*.items[0].stage) orelse 0);
}
...
pub fn instrumentProgram(self: *@This(), program: *Shader.Program, params: State.Params) !InstrumentationResult {
    ...
    if (re_link) {
        if (program.link) |l| {
            ...
            const result = l(declarations.shaders.ProgramPayload{
    ...
    return InstrumentationResult{
        .stages = stages,
        .invalidated = invalidated_any,
        .uniforms_invalidated = invalidated_uniforms,
    };
}
...
pub const InstrumentationResult = struct {
    ...
    pub fn toOpaque(self: *@This()) *declarations.instruments.Instrumentation {
        return @ptrCast(self);
    }

    pub fn toOpaqueConst(self: *const @This()) *const declarations.instruments.Instrumentation {
        return @ptrCast(self);
    }
};
...
pub const Shader = struct {
    ...
    const SourcesPayload: declarations.shaders.SourcesPayload = undefined;
    // SAFETY: not used in runtime
    ...
    pub const SourcePart = struct {
                ...
                if (comptime std.mem.eql(u8, fmt_str, "name")) {
                    if (self.source.tag) |t| {
                        ...
                        try writer.print(
                            "{}{s}",
                            .{
                                storage.Locator(Shader.SourcePart).PartRef{ .ref = self.source.ref, .part = self.part_index },
        ...
        pub fn basenameAlloc(self: *const SourcePart, allocator: std.mem.Allocator, part_index: ?usize) !String {
            ...
            if (self.tag) |t| {
                ...
                return try std.fmt.allocPrint(allocator, "{}{s}", .{
                    storage.Locator(Shader.SourcePart).PartRef{ .ref = self.ref, .part = found_part_index },
                    self.toExtension(),
                });
            }
        }
        ...
        pub fn init(allocator: std.mem.Allocator, payload: declarations.shaders.SourcesPayload, index: usize) !@This() {
        ...
        pub fn toPayload(allocator: std.mem.Allocator, parts: []Shader.SourcePart) !declarations.shaders.SourcesPayload {
            return declarations.shaders.SourcesPayload{
        ...
        pub fn freePayload(payload: declarations.shaders.SourcesPayload, allocator: std.mem.Allocator) void {
    ...
    pub fn instrumentStage(
        ...
        defer {
            ...
            while (it.next()) |entry| {
                entry.value.deinit(params.allocator);
            }
        }
        ...
        processor.config.source = preprocess: {
            ...
            break :preprocess try params.allocator.dupe(u8, std.mem.span(glslang.glslang_shader_get_preprocessed_code(shader)));
        };
        ...
        pub fn revertInstrumentation(self: *@This(), allocator: std.mem.Allocator) !void {
            ...
            while (it.next()) |shader_parts| {
                ...
                if (shader.compileHost) |compile| {
                    ...
                    const payload = declarations.shaders.SourcesPayload{
            ...
            pub fn next(self: *ShaderIterator) ?SourcePart.WithIndex {
                ...
                const found_part: usize = while (!found) {
                    if (self.current_parts == null or self.current_parts.?.len <= self.part_index) {
                        break while (self.shaders.next()) |n| { // find the first non-empty source
                            if (n.*.capacity > 0) {
                                self.current_parts = n.*.items;
                                self.part_index = 0;
                                stage = &self.current_parts.?[self.part_index];
...
pub fn sourcesCreateUntagged(service: *Service, sources: declarations.shaders.SourcesPayload) !void {
...
pub fn sourceSource(service: *Service, sources: declarations.shaders.SourcesPayload, replace: bool) !void {
                                ...
                                if (bp.id) |stop_id| { // if verified
                                    ...
                                    if (commands.instance != null and commands.instance.?.hasClient()) {
                                        commands.instance.?.sendEvent(
                                            .breakpoint,
                                            debug.BreakpointEvent{ .breakpoint = bp, .reason = .new },
                                        ) catch {};
                                    } else {
...
pub fn sourceType(service: *Service, ref: usize, stage: declarations.shaders.Stage) !void {
...
fn toGLSLangStage(stage: declarations.shaders.Stage) ShaderInfoForGLSLang {
    return switch (stage) {
