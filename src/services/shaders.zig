...
const State = struct {
    ...
    dirty: bool,
    params: Params,
    /// uses allocator from params. If outputs were null, no instrumentation was done
    outputs: instrumentation.Result.Outputs,
    //
    // Client debugger state. Does not affect the instrumentation
    //
    /// If source stepping is ongoing, this is the global index of the currently reached stop in `outputs.steps`.
    reached_step: ?u32 = null,
    /// Index into `State.outputs.steps`
    /// Set to non-null value to enable stepping.
    target_step: ?u32 = null,
    /// Slightly different meaning than target_step. Will be increased when user continues from a breakpoint or set to maximum value when breakpoint are disabled
    target_bp: u32 = 0,
...
pub fn stackTrace(allocator: std.mem.Allocator, args: debug.StackTraceArguments) !debug.StackTraceResponse {
    ...
    if (state.reached_step) |global_step| {
        ...
        const shader = locator.service.Shaders.all.get(locator.shader) orelse return error.TargetNotFound;
        const local_offset = if (state.outputs.parts_offsets) |pos| for (pos, 0..) |offset, part_index| {
        ...
        if (local_offset) |o| {
            const s = shader.items[reached_part].possible_steps.?.items(.pos)[global_step - o];
            try result.append(allocator, debug.StackFrame{
                .id = 0,
                .line = s.line,
                .column = s.character,
                .name = "main",
                .path = try locator.service.fullPath(allocator, &(shader).items[reached_part], null, reached_part),
            });
            if (levels > 1) {
                for (1..levels) |_| {
                    // TODO nesting
            ...
            log.warn("Reached global stop index {d} did not match any local stop.", .{global_step});
        }
    }
...
pub fn advanceBreakpoint(self: *@This(), shader_ref: usize, target: ?usize) !void {
...
pub fn advanceStepping(self: *@This(), shader_ref: usize, target: ?u32) !void {
    ...
    const state = self.state.getPtr(shader_ref) orelse return error.NotInstrumented;
    const last = state.reached_step orelse 0;

    if (target) |t| {
        state.target_step = t;
    } else {
        state.target_step = last +% 1;
    }

    // also invalidate instrumentation because stepping was previously disabled
    state.dirty = true;
...
fn instrumentOrGetCached(self: *@This(), program: *Shader.Program, params: *State.Params, key: usize, sources: []Shader.SourceInterface) !union(enum) {
    ...
    return find_cached: {
        ...
        if (result.outputs.diagnostics.capacity > 0) {
...
pub fn instrument(self: *@This(), program: *Shader.Program, in_params: State.Params) !InstrumentationResult {
    ...
    var state = std.AutoHashMapUnmanaged(usize, *State){};
...
pub const Shader = struct {
    ...
    pub const SourceInterface = struct {
        ...
        pub fn addBreakpoint(self: *@This(), bp: debug.SourceBreakpoint) !debug.Breakpoint {
            ...
            if (new.verified) {
                try self.breakpoints.put(self.allocator, new.id.?, {});
            } else {
                new.reason = "failed";
            }
