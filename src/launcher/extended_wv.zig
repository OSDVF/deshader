...
pub const State = struct {
    ...
    pub fn run(self: *@This(), seq: ZString, req: ZString) void {
        self.run_seq = seq;
        ...
        struct { // To wrap error handling
            fn run(a: Args, s: *State, command_s: ZString) !void {
                var list = std.ArrayListUnmanaged(String){};
                defer list.deinit(common.allocator);

                const exe = try common.selfExePathAlloc(common.allocator);
                defer common.allocator.free(exe);

                try list.append(common.allocator, exe);
                try list.append(common.allocator, a.program);
                ...
                log.info("Launched PID {d}", .{if (builtin.os.tag == .windows) C.GetProcessId(s.target.id) else s.target.id});

                var run_thread = try std.Thread.spawn(.{}, struct {
                    fn r(st: *State, command_seq: ZString) !void {
                ...
                }.r, .{ s, try common.allocator.dupeZ(u8, command_s) });
                defer run_thread.detach();
            }
        }.run(args.value[0], self, seq) catch |err| {
    ...
    pub fn injectFunctions(self: *@This()) void {
        ...
        self.view.bindRaw("run", self, run);
